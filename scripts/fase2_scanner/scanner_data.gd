...
static func challenges() -> Array[Dictionary]:
	return [
		...
		"hint": "O Scanner começa pela palavra reservada usada para declarar o tipo da variável.", "tokens": [
			_token("int", TokenKind.KEYWORD, "Palavra reservada utilizada para declarar uma variável inteira.", Vector2(510, 525)),
			_token("x", TokenKind.IDENTIFIER, "Nome utilizado para identificar uma variável.", Vector2(330, 370)),
			_token("=", TokenKind.OPERATOR, "Operador que atribui um valor à variável.", Vector2(1090, 315)),
			_token("10", TokenKind.NUMBER, "Valor numérico inteiro encontrado no código.", Vector2(835, 395)),
		...
		"hint": "Leia o código da esquerda para a direita. Depois de if vem o símbolo que abre a condição.", "tokens": [
			_token("if", TokenKind.KEYWORD, "Palavra-chave que inicia uma estrutura condicional.", Vector2(650, 255)),
			_token("(", TokenKind.SYMBOL, "Símbolo que abre a condição do if.", Vector2(330, 370)),
			_token("x", TokenKind.IDENTIFIER, "Identificador usado na condição.", Vector2(1080, 315)),
			_token(">", TokenKind.OPERATOR, "Operador utilizado para realizar uma comparação.", Vector2(835, 395)),
			_token("10", TokenKind.NUMBER, "Valor numérico comparado com o identificador.", Vector2(510, 525)),
			_token(")", TokenKind.SYMBOL, "Símbolo que fecha a condição do if.", Vector2(1080, 540)),
			_token("return", TokenKind.KEYWORD, "Palavra-chave que devolve um valor.", Vector2(215, 510)),
			_token("x", TokenKind.IDENTIFIER, "Identificador cujo valor será devolvido.", Vector2(670, 565)),
			_token(";", TokenKind.SYMBOL, "Símbolo que encerra a instrução return.", Vector2(925, 525)),
		]},
	]
